...
#### 8.1 出力順序の問題
- [ ] `src/output.py`での出力順序の確認
- [ ] コマの読み順が正しく反映されているかの検証
- [ ] 同じ段内での右→左の順序が正しいかの確認

#### 8.2 修正実装
- [ ] `region_id`（`reading_order`）でのソートが正しく機能しているかの確認
- [ ] コマのbboxの上端y座標で段を判定した結果が正しく反映されているかの確認
- [ ] 必要に応じて出力順序の修正

**完了条件**: 同じ段内で右→左の順序が正しく反映されている
